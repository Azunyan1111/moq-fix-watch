...
pub struct Diff {
	...
	/// should publish a full snapshot instead. This happens when a changed value is (or
	/// contains) a JSON array, since merge patch can only replace arrays wholesale, or when a
	/// value is set to JSON null, which merge patch reads as a key deletion.
...
fn diff_objects(old: &Map<String, Value>, new: &Map<String, Value>, patch: &mut Map<String, Value>, forced: &mut bool) {
	...
	for (key, new_val) in new {
		...
		// Added or replaced with a non-object value. A literal null can't be stored (merge
		// patch would delete the key), and any array means we can't express a partial edit.
		if new_val.is_null() || contains_array(new_val) || old_val.is_some_and(contains_array) {
			*forced = true;
		}
...
/// Whether a value is a JSON array or contains one nested inside an object.
...
mod test {
	...
	#[test]
	fn changed_array_forces_snapshot() {
		let result = diff(&json!({ "a": [1, 2] }), &json!({ "a": [1, 2, 3] }));
		assert!(result.forced_snapshot);
	}

	#[test]
	fn added_array_forces_snapshot() {
		let result = diff(&json!({ "a": 1 }), &json!({ "a": 1, "b": [1] }));
		assert!(result.forced_snapshot);
	}

	#[test]
	fn nested_array_forces_snapshot() {
