...
mod test {
	...
	#[test]
	fn array_change_rolls_snapshot() {
		let config = JsonConfig {
			max_delta_ratio: Some(100.0),
		};
		let (mut producer, track) = producer(config);
		...
		// The array change can't be a delta, so a second snapshot group is created.
		assert_eq!(track.latest(), Some(1));
	...
	fn newer_group_supersedes_in_progress_reconstruction() {
		...
		// Force a brand-new snapshot group, then keep publishing into it.
		producer.update(&json!({ "list": [1] })).unwrap(); // array => rolls a new group
		producer.update(&json!({ "list": [1], "z": 9 })).unwrap();
		producer.finish().unwrap();
		...
		assert_eq!(last.unwrap(), json!({ "list": [1], "z": 9 }));
	}
}
