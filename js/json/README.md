...
Pass `{ maxDeltaRatio: x }` to `Producer` to emit merge-patch deltas while a group stays within `x` times the size of a fresh snapshot. Array or null changes always fall back to a snapshot, since merge patch can't express them.
