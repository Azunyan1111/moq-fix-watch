...
export interface Diff {
	...
	// publish a full snapshot instead. This happens when a changed value is (or contains) an
	// array, since merge patch replaces arrays wholesale, or when a value is set to null, which
	// merge patch reads as a key deletion.
...
function diffObjects(oldObj: Obj, newObj: Obj, patch: Obj, forced: { value: boolean }): void {
	...
	for (const key of Object.keys(newObj)) {
		...
		// would delete the key), and any array means we can't express a partial edit.
		if (newV === null || containsArray(newV) || (inOld && containsArray(oldV))) {
			forced.value = true;
		}
		patch[key] = newV;
	}
...
// Whether a value is an array or contains one nested inside an object.
function containsArray(value: unknown): boolean {
	if (Array.isArray(value)) return true;
	if (isObject(value)) return Object.values(value).some(containsArray);
	return false;
}
