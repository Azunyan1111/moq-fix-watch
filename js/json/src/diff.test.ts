...
test("changed array forces snapshot", () => {
	expect(diff({ a: [1, 2] }, { a: [1, 2, 3] }).forcedSnapshot).toBe(true);
});

test("added array forces snapshot", () => {
	expect(diff({ a: 1 }, { a: 1, b: [1] }).forcedSnapshot).toBe(true);
});

test("nested array forces snapshot", () => {
	expect(diff({ o: { x: 1 } }, { o: { x: 1, list: [1] } }).forcedSnapshot).toBe(true);
});
